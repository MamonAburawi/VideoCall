...
class RTCClient(
    ...
    companion object {
        ...
        private const val CALL_STATUS_FIELD = "callStatus"
            ...
            override fun onCreateSuccess(desc: SessionDescription?) {
                GlobalScope.launch(Dispatchers.IO) {
                    ...
                    if (callRef != null) {
                        ...
                        data?.callStatus = desc?.type.toString()
                        data?.sdp = desc?.description.toString()
                        data?.startAt = Date()

                        db.collection(CALLS_COLLECTION).document(data!!.callId)
    ...
    suspend fun endCall(meetingID: String) {
        ...
        if (candidates != null){
            ...
            // update type
            val endCall = hashMapOf(CALL_STATUS_FIELD to CallState.END_CALL)
