...
class EncounterViewModel(application: Application): AndroidViewModel(application) {
    ...
    companion object{
        const val TAG = "EncounterViewModel"
    }
    ...
    private val _isAnswer = MutableLiveData<Call?>()
    val isAnswer: LiveData<Call?> = _isAnswer
    ...
    fun joinToEncounterDone(){

        _isAnswer.value = null
    }

    fun listenerToCalls(userId: String){
        val root = Firebase.firestore
        root.collection("user").document(userId)
            .collection("calls").addSnapshotListener { value, error ->
                if (error == null){
                    if (!value!!.isEmpty){
                        val call = value.toObjects(Call::class.java)[0]
                        _isAnswer.value = call
